...
def convert_to_transformer_inputs(instance, tokenizer, max_sequence_length):
...
class Ontology:
    ...
    def onto2ids(self):
        ...
        #     for slot in self.ontology_dict])
        #slots_values {”病症“：[同义词x35的句子]，”检查“: [同义词x16的句子] 。。。}
        max_len = 35
        slots_values = dict()
        ...
        for slot in self.ontology_dict.keys():
            for value in self.ontology_dict[slot]:
                ...
                if len(self.examples[slot][value]) != 0:
                    ...
                    seq += ')'
                length = len(seq)
                seq = convert_to_transformer_inputs(seq,self.tokenizer,max_len)[0]
                try:
...
class Data:
    def __init__(self, max_len, ontology, tokenizer):
    ...
    def _add_window(self, name, fname):
        ...
        for dialog in dialogs:
            for window in dialog:
                ...
                window_utts = [convert_to_transformer_inputs(utt,self.tokenizer, self.max_len)[0] \
                    for utt in window_utts]
                self.datasets[name]['windows_utts'].append(window_utts)
                label = window['label']
                label = filter(lambda x: '药物' not in x, label)
                label_indexes = self.ontology.labels2indexes(texts = label, style = 'window')
        ...
        for dialog in dialogs:
            for window in dialog:
                ...
                for i,utt in enumerate(utts):
                    if len(utt) != 0 and i != len(utt)-1:
                        chunk_seq.append((utt+'<seg>'))
                    else:
                        chunk_seq.append(utt)
                chunk_seq_len = len(chunk_seq)
                chunk_seq_ids = convert_to_transformer_inputs(chunk_seq, self.tokenizer, self.max_len)[0]
